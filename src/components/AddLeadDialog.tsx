...
import { autoAssignAgent } from '@/lib/autoAssign';
...
const AddLeadDialog = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const agentId = form.assigned_agent_id || (await autoAssignAgent()) || agents?.[0]?.id || null;
      ...
      const assignedName = agents?.find(a => a.id === agentId)?.name || 'an agent';
      toast.success(`Lead created! Score: ${score}/100 — assigned to ${assignedName}`);
      setOpen(false);
      setDuplicate(null);
      setParsed(null);
      setRawText('');
