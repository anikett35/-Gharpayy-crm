...
export const useUpdateLead = () => {
  ...
  return useMutation({
    mutationFn: async ({ id, ...updates }: { id: string } & Database['public']['Tables']['leads']['Update']) => {
      // Fetch current lead to merge fields for scoring
      const { data: current } = await supabase.from('leads').select('*').eq('id', id).single();
      const merged = { ...current, ...updates };

      // Recalculate score whenever status, budget, or location changes
      const shouldRecalculate = updates.status || updates.budget !== undefined || updates.preferred_location !== undefined;
      if (shouldRecalculate && merged) {
        ...
        updates.lead_score = calculateLeadScore({
          ...
          preferred_location: merged.preferred_location,
          last_activity_at: merged.last_activity_at,
          visit_count: (visitCount as any)?.count ?? 0,
        });
      }

      const { data, error } = await supabase.from('leads').update({ ...updates, last_activity_at: new Date().toISOString() }).eq('id', id).select().single();
      if (error) throw error;
      return data;
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ['leads'] }),
